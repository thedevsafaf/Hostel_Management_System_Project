...
namespace Hostel_Management_System_Project
{
    public partial class ViewParentsList : System.Web.UI.Page
    {
        ...
        void DisplayParentsList()
        {
            ...
            string query = @"
                    SELECT 
                          ROW_NUMBER() OVER (ORDER BY pt.parent_id) AS sl_no, pt.parent_id, 
                          pt.login_id, pt.name, pt.email, pt.phone_number, 
                          pt.student_id, st.name as student_name, pt.status, pt.created_at 
                    FROM 
                          parent_table pt 
                    INNER JOIN 
