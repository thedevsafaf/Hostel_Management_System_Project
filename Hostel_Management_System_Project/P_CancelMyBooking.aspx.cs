...
{
    public partial class P_CancelMyBooking : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                // Call a method to populate bookings list
        ...
        private void PopulateMyBookingList()
        {
            using (SqlConnection con = new SqlConnection(@"Data Source=DESKTOP-JRHVVPL\SQLEXPRESS;Initial Catalog=hostel_db;Integrated Security=True"))
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT ROW_NUMBER() OVER (ORDER BY bt.booking_id) AS SerialNumber, bt.booking_id, bt.student_id, bt.booking_date, bt.status AS booking_status, rf.room_id, rf.room_no, rf.room_desc FROM booking_table bt LEFT JOIN room_facilities rf ON bt.room_id = rf.room_id INNER JOIN student_table st ON bt.student_id = st.student_id inner join parent_table prt on prt.student_id = st.student_id where prt.parent_id = @parent_id AND bt.status = 'Confirmed';", con);
        ...
        protected void btn_Cancel_Click(object sender, EventArgs e)
        {
            ...
            //PopulateMyBookingList();
            Response.Redirect("P_ViewMyBooking.aspx");
